...
//
// IMPORTANT:
// The player house door intentionally remains at x:270, y:2200 because
// the existing tests expect the house door there.
...
function nearRect(lx, ly, r, pad) {
  return lx >= r.x - pad && lx < r.x + r.w + pad &&
...
function bakeInto(g, lx0, ly0) {
  ...
  for (let y = 10; y < H; y += 72) {
    g.fillRect(MAIN_NS_X + MAIN_NS_W / 2 - 4, y, 8, 34);
  }

  for (let x = 10; x < W; x += 72) {
    g.fillRect(x, MAIN_EW_Y + MAIN_EW_H / 2 - 4, 34, 8);
  }
  ...
  // Top-left parking/park district
  ...
  for (const x of [1415, 1665, 1915, 2165, 2410]) {
    ...
    building(x, 165, 185, 205);
    g.fillStyle = '#144f6a';
    g.fillRect(x + 125, 370, 24, 44);
  }

  building(1415, 735, 185, 210, 'Chief\nof\nPolice');

  for (const x of [1665, 1915, 2165, 2410]) {
    lot(x - 18, 685, 205, 295);
    building(x, 735, 185, 210);
  }

  // Dense lower-left neighborhood
  building(35, 1310, 185, 190, 'Creepy\nHouse');

  for (const x of [245, 455, 665, 875, 1085]) {
    building(x, 1310, 185, 190);
  }

  for (const x of [35, 245, 455, 665, 875, 1085]) {
    building(x, 1725, 185, 190, x === 875 ? 'School\nPrincipal' : '');
  }

  for (const x of [35, 245, 455, 665, 875, 1085]) {
    building(x, 2135, 185, 190, x === 1085 ? 'Friend' : '');
  }
...
function minimapBake() {
  ...
  const miniBuildings = [
    ...
    [1415, 165, 185, 205], [1665, 165, 185, 205],
    [1915, 165, 185, 205], [2165, 165, 185, 205],
    [2410, 165, 185, 205],

    [1415, 735, 185, 210], [1665, 735, 185, 210],
    [1915, 735, 185, 210], [2165, 735, 185, 210],
    [2410, 735, 185, 210],

    [35, 1310, 185, 190], [245, 1310, 185, 190],
    [455, 1310, 185, 190], [665, 1310, 185, 190],
    [875, 1310, 185, 190], [1085, 1310, 185, 190],

    [35, 1725, 185, 190], [245, 1725, 185, 190],
    [455, 1725, 185, 190], [665, 1725, 185, 190],
    [875, 1725, 185, 190], [1085, 1725, 185, 190],

    [35, 2135, 185, 190], [245, 2135, 185, 190],
    [455, 2135, 185, 190], [665, 2135, 185, 190],
    [875, 2135, 185, 190], [1085, 2135, 185, 190],

    [210, 2020, 230, 200],

    [1400, 1325, 300, 205], [1840, 1325, 300, 205],
    [2280, 1325, 300, 205],

    [1410, 1835, 300, 205], [1845, 1835, 300, 205],
    [2285, 1835, 300, 205],
  ];

  for (const b of miniBuildings) B(b[0], b[1], b[2], b[3], '#78c3df');

  // Player/test-compatible house marker
  g.fillStyle = '#ffc44d';
  g.beginPath();
  g.arc(302 * SC, 2244 * SC, 3, 0, Math.PI * 2);
  g.fill();
